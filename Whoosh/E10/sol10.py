...
def extraer_pelicula(url):
    ...
    for link_pelicula in lista_link_peliculas:
        ...
        if(datos.find("dt",string="PaÃ­s")):
            # los espacios en blanco los sustituimos por _ para poder hacer bÃºsquedas en las keywords
            pais = "".join(datos.find("dt",string="PaÃ­s").find_next_sibling("dd").stripped_strings).replace(" ","_")
        else:
            pais="Desconocido"
        ...
        # los espacios en blanco los sustituimos por _ para poder hacer bÃºsquedas en las keywords
        generos = "".join(generos_director.find("p",class_="categorias").stripped_strings).replace(" ","_")
        director = "".join(generos_director.find("p",class_="director").stripped_strings).replace(" ","_")
...
def almacenar_datos():
    ...
    schem = Schema(titulo=TEXT(stored=True), titulo_original=TEXT(stored=True), pais=KEYWORD(stored=True,commas=True,lowercase=True), fecha=DATETIME(stored=True), director=KEYWORD(stored=True,commas=True,lowercase=True), generos=KEYWORD(stored=True,commas=True,lowercase=True), sinopsis=TEXT(stored=True), url=ID(stored=True,unique=True))
    ...
    def mostrar_lista(event):
        ...
        with ix.searcher() as searcher:
            ...
            for r in results: 
                ...
                lb.insert(END,r['director'].replace('_',' '))
                lb.insert(END,'')
...
def buscar_generos():
    def mostrar_lista(event):
        ...
        with ix.searcher() as searcher:
            ...
            print(lista_generos)
            # en la entrada sustituimos los espacios por _ y ponemos todo en minÃºsculas
...
def modificar_fecha():
    def modificar():
        ...
        if respuesta:
            writer = ix.writer()
            for r in lista:
                writer.update_document(url=r['url'], fecha=str(en1.get()), titulo=r['titulo'], titulo_original=r['titulo_original'], pais=r['pais'], director=r['director'], generos=r['generos'], sinopsis=r['sinopsis'])
            writer.commit()
