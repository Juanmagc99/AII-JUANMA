...
def almacenar_bd():
    ...
    for i in l:
        ...
        print("Esta es la jornada: " + str(jornada))
        ...
        for p in partidos:
            ...
            if resultado_enlace != None:
                '''.* es cualquie caracter menosespacio las veces que sea, el string coge lo que tengas
                entre las etiquetas html y con strip quitas los espacios, en goles no usamos llamada a id o class
                porque esta fuera de las etiquetas html'''
                goles=re.compile('(\d+).*(\d+)').search(resultado_enlace.string.strip())
                #Group 0 da el patron completo///PREGUNTAR EN PRACTICAS QUE HACE EL GROUP
